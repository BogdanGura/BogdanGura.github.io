...
window.addEventListener("DOMContentLoaded", () => {
    ...
    btnUp.addEventListener("click", checkUp);
    btnDown.addEventListener("click", checkDown);
    btnLeft.addEventListener("click", checkLeft);
    btnRight.addEventListener("click", checkRight);
...
function createAchievements() {
    for (let i = 0; i < achievementNames.length; i++) {
        let messageOnCompletion = `You earned ${achievementNames[i]}`;
...
// Functions for setting direction for html buttons

//Runs when up button is pressed
function checkUp() {
    //if snake's direction is up
    //then ignore the press
    //register everithing else
    if(direction !== +width)
    {
        direction = -width;
    }
